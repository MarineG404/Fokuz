		...
		<BlockCard style={styles.container}>
			<View style={styles.header}>
				<View style={styles.methodInfo}>
					<Ionicons name={getMethodIcon(session.methodName)} size={20} color={COLORS.primary} style={styles.methodIcon} />
			...
			<View style={styles.statsContainer}>
				...
				<View style={styles.statItem}>
					...
					<Text style={[styles.statValue, { color: COLORS.text }]}>{historyService.formatDuration(session.totalWorkTime)}</Text>
				...
				{session.totalBreakTime > 0 && (
					<View style={styles.statItem}>
						...
						<Text style={[styles.statValue, { color: COLORS.text }]}>{historyService.formatDuration(session.totalBreakTime)}</Text>
			...
			<View style={styles.footer}>
				<Text style={[styles.timeRange, { color: COLORS.textSecondary }]}>{formatTime(session.startTime)} → {formatTime(session.endTime)}</Text>
			</View>
		</BlockCard>
	);
};
...
const styles = StyleSheet.create({
	...
	containerDark: { shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 3 },
